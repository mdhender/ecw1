...
| Conflicts and gaps (`docs/conflicts-and-gaps.md`) | **Not a source**         | Tracking document. Holds [§20](./conflicts-and-gaps.md#20-source-conflicts) and [§21](./conflicts-and-gaps.md#21-gaps), which record what is unsettled about the rules stated here. Every `CONFLICT-nn` and `GAP-nn` tag in this manual resolves there. Settles nothing on its own; only a decision in [§0.4](#04-decisions-on-record) does. |
...
- **Facts** are stated as facts: "A mine removes `25 × TL` ore per turn." Everything in [§1](#1-universe-model) through
  [§23](#23-constants-and-limits) is a fact of the game unless it carries one of the two tags below.
- **Gaps** — rules the sources do not supply — are tagged `GAP-nn`. A gap is a decision the design owner must make.
  Implementers must not invent a rule for a gap; they must escalate it.
...
| `D-24` | **Every game starts at Tech Level 1 in every item type.** The starting Tech Level is not configurable, per game or per player. | Closes the third clause of `GAP-03`, which is restated with the other two standing. Overrides the handbook, which says that in some games players may start above `TL-1` for some item types; that flexibility is not adopted. It is what lets `D-21` and `D-23` state the starting position as one set of numbers instead of a family of them, because every statistic in the game is a function of its own `TL` ([§0.5](#05-notation-and-conventions)) and almost every quantity in the seed would move if the level moved. The constraints that make the seed close are `TL`-1 constraints in particular: a `Life Support-1` sustains exactly one population unit because capacity is `TL^2`, one `Structure-1` encloses exactly `1 / StructureRatio`, and a `Farms-1` produces 25 food where every higher farm produces `5 × TL`, which is the one place the progression is not smooth ([§7.2](#72-farms)). A game starting above `TL` 1 would need its own seed, not a scaled one. Advancement is unaffected: a player still climbs one `TL` at a time from 1 ([§8.1](#81-tech-levels)), and may still hold higher-`TL` items taken by capture or trade. |
...
that document; neither is authoritative until a decision in [§0.4](#04-decisions-on-record) closes it.
